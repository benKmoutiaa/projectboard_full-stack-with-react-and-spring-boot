...
{
    try{
        await axios.post("http://localhost:8090/api/project",project_task);
        history.push("/");
         dispatch({
            type:GET_ERRORS,
             payload:{}
...
export const deleteProjectTask = pt_id => async dispatch => {
    if (
      ...
      await axios.delete(`http://localhost:8090/api/project/${pt_id}`);
      dispatch({
  ...
  export const getProjectTask = (pt_id, history) => async dispatch => {
    try {
      const res = await axios.get(`http://localhost:8090/api/project/${pt_id}`);
      dispatch({
